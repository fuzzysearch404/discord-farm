...
from botsettings import bot_auth_token
...
CLUSTER_NAMES = (
    'Apple', 'Banana', 'Broccoli', 'Cabbage', 'Cacao', 'Cherry', 'Coconut', 'Corn',
    'Cotton', 'Cranberry', 'Cucumber', 'Eggplant', 'Garlic', 'Ginger', 'Grape', 'Hazelnut',
    'Indigo', 'Jasmine', 'Lettuce', 'Lime', 'Mango', 'Mushroom', 'Nectarine', 'Olive', 'Orange'
)
...
class Launcher:
    ...
    def get_shard_count(self):
        data = requests.get('https://discordapp.com/api/v7/gateway/bot', headers={
            ...
            "User-Agent": "Discord Farm Bot (https://github.com/Rapptz/discord.py 1.3.3) Python/3.7 aiohttp/3.6.2"
        })
...
class Cluster:
    ...
    async def start(self, *, force=False):
        ...
        kw = self.kwargs
        kw['pipe'] = stdin
        self.process = multiprocessing.Process(target=BotClient, kwargs=kw, daemon=True)
        self.process.start()
